...
事实证明，线程和任务往往能很好地一起工作，因为任务可以（至少在某些运行时下）在线程之间迁移。事实上，在底层，我们一直在使用的运行时 -- 包括 `spawn_blocking` 和 `spawn_task` 函数 -- 默认就是多线程的！许多运行时采用一种称为 *工作窃取，work stealing* 的方法，根据线程当前的利用情况，在线程之间透明地迁移任务，从而提升系统的整体性能。这种方法实际上同时需要 *线程* 和 *任务*，因此也需要未来值。
