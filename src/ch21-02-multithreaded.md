...
在这里，我们首选对 `execute` 调用 lock 来获取互斥量，然后调用 unwrap 来对任何错误终止运行。当互斥量处于 *中毒* 状态时，则获取锁可能失败，这种情况会在某个其他线于持有锁期间终止运行，而非释放锁时发生。在这种情况下，调用 unwrap 来让这个线程终止运行，便是要采取的正确操作。咱们可以修改这个 unwrap 为带有对咱们有意义的报错信息的 expect。
