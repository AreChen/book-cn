...
我们已经讨论了怎样传递闭包给函数；咱们也可以传递常规函数给函数！当咱们打算传递某个已定义好的函数，而不是定义新的闭包时，这种技巧非常有用。函数会强制转换为 `fn` 类型（`Fn` 小写），而不会与 `fn` 的闭包特质混淆。fn 类型被称为 *函数指针，funciton pointer*。通过函数指针传递函数将允许咱们把函数作为其他函数的参数使用。
